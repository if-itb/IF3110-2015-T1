...
- Validasi dilakukan dengan menggunakan javascript. Pertama-tama saya membuat fungsi pada javascript yang bisa memeriksa apakah sebuah field itu kosong atau tidak. Untuk validasi email dibuat sebuah regular expression untuk email yang harus memiliki @ dan .
- Untuk validasi penghapusan saya membuat fungsi di javascript yang mengembalikan nilai dari fungsi confirm().
- Untuk AJAX, ketika pengguna mengklik tombol arrow up untuk question maka fungsi vote() pada javascript akan menerima ID berupa hasil QUID dimana huruf pertama menandakan panah yang diklik adalah untuk question atau answer(Q atau A), huruf kedua menandakan up vote atau down vote(U atau D) dan yang terakhir adalah nomor ID dari question ataupun answer. Setelah diparsing maka akan dikirimkan ke file PHP-nya dengan xmlhttp.open() lalu xmlhttp.send(). Setelah diterima di file PHP-nya maka akan diperiksa apakah question atau answer dan apakah up atau down lalu di-update di database dan file PHP ini akan mengeluarkan nilai yang baru dengan echo yang akan diterima oleh fungsi vote() yang tadi melalui xmlhttp,response() lalu diupdate ke tampilan
