...
function deleteQuestion(context, id) {
    var a = confirm("Anda yakin ingin menghapus pertanyaan ini?");

    if ( a == true ) {
    	window.location.href = context + "/delete/question/" + id;
    }
}
    ...
    if (id == "" || id == null || type == "" || type == null) {
        ...
        xmlhttp.onreadystatechange = function() {
            if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
         		...
         		if ( q_a == "answer" ) {
                            document.getElementById("vote_count_a_" + id).innerHTML = i + parseInt(document.getElementById("vote_count_a_" + id).innerHTML);
                        } else if ( q_a == "question") {
                            document.getElementById("vote_count_q_" + id).innerHTML = i + parseInt(document.getElementById("vote_count_q_" + id).innerHTML);
                        }
            }
        }
